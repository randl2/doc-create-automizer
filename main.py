...
# ================= 0. ВСТАНОВЛЕННЯ БРЕНДОВИХ ШРИФТІВ (TAHOMA ТОЩО) =================
if sys.platform != "win32":
    # Шлях до системної папки користувацьких шрифтів у Linux
    ...
    if os.path.exists("fonts"):
        ...
        for font_file in os.listdir("fonts"):
            if font_file.lower().endswith((".ttf", ".otf")):
                src = os.path.join("fonts", font_file)
                dst = os.path.join(system_fonts_dir, font_file)
                shutil.copy(src, dst)

        # Оновлюємо кеш шрифтів операційної системи Linux
...
st.caption("Завантажте Word-шаблон і Excel-таблицю, щоб отримати готовий архів із PDF-файлами.")[cite: 2]
