...
st.set_page_config(
    page_title="Docx to PDF Batch Generator", page_icon="📄", layout="centered"
)

st.title("📄 Генератор документів у PDF з архівацією в ZIP")
st.caption(
    "Завантажте Word-шаблон і Excel-таблицю, щоб отримати готовий архів із PDF-файлами."
)
...
def clean_and_replace_run_text(paragraphs, company, materials):
  for p in paragraphs:
    # 1. Склеюємо розірвані Вордом плейсхолдери (якщо вони побилися на шматки)
    i = 0
    while i < len(p.runs) - 1:
      if any(
          part in p.runs[i].text for part in ["Company", "Material", "_name"]
      ):
    ...
    # 2. Заміна тексту із збереженням стилів шаблону
    for run in p.runs:
...
# ================= 3. ЗАВАНТАЖЕННЯ ФАЙЛІВ У БРАУЗЕРІ =================
...
with col2:
  uploaded_excel = st.file_uploader(
      "Таблиця даних (.xlsx)", type=["xlsx", "xls"], key="excel_uploader"
  )
...
if uploaded_template and uploaded_excel:
  try:
    ...
    # Автопошук колонок для назв
    company_col = next(
        (
            c
            for c in df.columns
            if any(
                k in c.lower() for k in ["company", "назва", "організац", "компан"]
            )
        ),
        None,
    )
    material_col = next(
        (
            c
            for c in df.columns
            if any(k in c.lower() for k in ["material", "матеріал", "продукц"])
        ),
        None,
    )
    ...
    if not company_col:
      st.error(
          f"❌ У таблиці не знайдено колонки з компанією. Доступні колонки:"
          f" {list(df.columns)}"
      )
    else:
      df_clean = df.dropna(subset=[company_col]).copy()
      st.write(
          f"📊 Знайдено рядків для формування документів: **{len(df_clean)}**"
      )
      ...
      if st.button("🚀 Згенерувати PDF та створити ZIP-архів", type="primary"):
        ...
        # Використовуємо тимчасову робочу папку
        with tempfile.TemporaryDirectory() as temp_dir:
          # Зберігаємо файл шаблону на диск
          ...
          for index, (_, row) in enumerate(df_clean.iterrows()):
            ...
            # Очищення імені від недопустимих символів для назви файлу Windows
            safe_company = "".join(
                c for c in company if c.isalnum() or c in (" ", "_", "-")
            ).rstrip()
            temp_docx = os.path.join(temp_dir, f"temp_{safe_company}.docx")
            output_pdf = os.path.join(
                temp_dir, f"Пропозиція партнерства {safe_company}.pdf"
            )

            status_text.text(f"Обробка [{index + 1}/{total}]: {company}...")

            # 1. Заміна плейсхолдерів
            replace_placeholders(
                template_path, company, materials, temp_docx
            )
          ...
          # ================= 6. ЗБИРАННЯ ZIP-АРХІВУ В ПАМ'ЯТІ =================
          if generated_pdf_paths:
            ...
            st.balloons()
            st.success(
                f"🎉 Успішно створено {len(generated_pdf_paths)} PDF-файлів!"
            )

            # Кнопка для скачування архіву
            st.download_button(
    ...
    st.error(f"Помилка зчитування таблиці або шаблону: {e}")

else:
  st.info("👆 Будь ласка, завантажте Word-шаблон та файл Excel для початку.")